...
import { getStreams, getCatalog, getMeta, makeManifest, CATALOGS } from './src/stremio.js';
...
import { safeMessage } from './src/utils.js';

const PORT = Number(process.env.PORT || 3000);
const DIRECT_VERSION = '1.0.0';
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='GET'&&path==='/health'){
      json(res,200,{ok:true,addon:'KRA Direct',version:DIRECT_VERSION,node:process.version,direct:true,cder:false,configSecurity:configSecurityMode(),tmdb:tmdbConfigured(),csfd:csfdConfigured(),at:new Date().toISOString()});return;
    }
    if(req.method==='GET'&&path==='/manifest.json'){json(res,200,manifest(false));return;}
    if(req.method==='POST'&&path==='/api/configure'){
      const b=await readJson(req);
      const config={username:String(b.username||'').trim(),password:String(b.password||''),preferredLanguages:Array.isArray(b.preferredLanguages)?b.preferredLanguages.map(String).slice(0,4):['sk','cs'],maxStreams:Math.max(1,Math.min(20,Number(b.maxStreams)||10)),uid:crypto.randomUUID()};
    ...
    const mm=path.match(/^\/([^/]+)\/manifest\.json$/);
    if(req.method==='GET'&&mm){decodeConfig(mm[1]);json(res,200,manifest(true));return;}

    const cm=path.match(/^\/([^/]+)\/catalog\/(movie|series)\/([^/]+)(?:\/([^/]+))?\.json$/);
    if(req.method==='GET'&&cm){
      const [,token,type,catalogId,extraRaw]=cm;const config=decodeConfig(token);const extra={};
      if(extraRaw){for(const[k,v]of new URLSearchParams(extraRaw).entries())extra[k]=v;}
      const result=await getCatalog(config,type,catalogId,extra);
      json(res,200,{metas:Array.isArray(result?.metas)?result.metas:[]});return;
    }

    const metam=raw.match(/^\/([^/]+)\/meta\/(movie|series)\/([^/]+)\.json$/);
    if(req.method==='GET'&&metam){
      const [,tokenRaw,type,idRaw]=metam;const config=decodeConfig(decodeURIComponent(tokenRaw));const id=decodeURIComponent(idRaw);
      let meta=await getMeta(config,type,id);
      meta=await enrichMetaWithTmdb(type,id,meta);
      meta=await enrichMetaWithCsfd(type,id,meta);
      if(meta)meta={...meta,id,type};
      json(res,200,{meta:meta||null});return;
    }

    const sm=raw.match(/^\/([^/]+)\/stream\/(movie|series)\/([^/]+)\.json$/);
    if(req.method==='GET'&&sm){
      const [,tokenRaw,type,idRaw]=sm;const config=decodeConfig(decodeURIComponent(tokenRaw));const id=decodeURIComponent(idRaw);
      const result=await getStreams(config,type,id);
      json(res,200,{streams:decorateStreams(Array.isArray(result?.streams)?result.streams:[])});return;
    }

    const dm=path.match(/^\/([^/]+)\/diagnostics\.json$/);
    if(req.method==='GET'&&dm){
      const config=decodeConfig(dm[1]);const kra=new KraClient(config);const info=await kra.userInfo();const sc=new StreamCinemaClient(config,kra);await sc.getAuthToken();
      const out={ok:true,direct:true,cder:false,version:DIRECT_VERSION,kra:{login:true,username:info?.data?.username||config.username,daysLeft:info?.data?.days_left??null},catalogs:CATALOGS.length,at:new Date().toISOString()};
      const catalog=url.searchParams.get('catalog');if(catalog){const type=catalog.includes('series')?'series':'movie';try{const r=await getCatalog(config,type,catalog,{skip:0});out.catalog={id:catalog,type,count:r.metas?.length||0,diagnostics:r.diagnostics||null};}catch(e){out.ok=false;out.catalog={id:catalog,error:safeMessage(e)};}}
      const type=url.searchParams.get('type'),id=url.searchParams.get('id');if((type==='movie'||type==='series')&&id){try{const r=await getStreams(config,type,id);out.stream={type,id,count:r.streams?.length||0,diagnostics:r.diagnostics||null};}catch(e){out.ok=false;out.stream={type,id,error:safeMessage(e)};}}
